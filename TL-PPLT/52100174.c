...
#include<string.h>
#include<ctype.h>
//DA CHECK LIST MALE,COUNT,REPORT CHu tim thay tc \n,da check average,da check sort ,da check country
typedef struct{
    int Id_2,Pro;
...
int main(){
    ...
    Nhap(Info,&len);
    gets(x);
    memcpy(country_test,x,strlen(x));
    memcpy(temp,x,strlen(x));
    Vietstrlwr(temp);

    if(strncmp(temp,"country",7)==0 && res_count==0){
        res_count=1;
    ...
    if(strncmp(temp,"count",5)==0 && res_count==0){
        res_count=1;
        if(CheckCount(temp)==0){
            res=0;
        }else{
